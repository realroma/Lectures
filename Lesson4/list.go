...
func (l *List) Reverse() *List {
	...
	for l.root.next.Val != nil {
		...
		//Сначала у нас имеется вид вхождения переменной в цикл "prev val next" мы меняем значения местами и переходим в следующий, ранее смещённый на позицию prev
		//Так как при переходе, у нас снова структура вида "prev val next", условие должно быть установлено на next.Val так как в обратном случае, оно будет
		//брать значение предидущей структуры и уходить в бесконечный цикл.
		Swap(l.root)
		l.root = l.root.prev
	}
	Swap(l.root)
	...
	//Оно здесь не нужно. Буфер нужен только в конце для перестановки начального указателя.
}
...
//Здесь была ошибка с написанием. Так же тут используются значения по ссылке, имеющие очень чувствительную к изменениям ячейку расположения.
